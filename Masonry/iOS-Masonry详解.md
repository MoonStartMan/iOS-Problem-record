...
[self.blueView mas_makeConstraints: ^(MASConstratintMaker *make){
...
/**
	Masonry为我们提供了三个默认的方法，priorityLow()、priorityMedium()、priorityHigh()，这三个方法内部想对应着不同的默认优先级。
...
[self.redView mas_makeConstraints: ^(MASConstraintMaker *make) {
	...
	make.width.equalTo(self.view).mutipliedBy(0.2);
}];
